...
import { getGithubActivity } from "../services/githubService.ts";
import { getLeetcodeActivity } from "../services/leetcodeService.ts";
import { getActivity } from "../services/activityService"; // NEW
import ActivityHeatmap from "../components/ActivityHeatmap"; // NEW
...
function Profile() {
  ...
  // NEW — app's own streak, replaces the hardcoded 18
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // NEW — fetch the app's own streak, independent of the tasks above
        tasks.push(
          getActivity()
            .then((activityData) => {
    ...
    <div className="min-h-screen bg-gray-900 px-6 py-8 text-white">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-xl bg-gray-800 p-5 text-center">
            <p className="text-2xl">🔥</p>
            {/* CHANGED — real app streak instead of hardcoded 18 */}
